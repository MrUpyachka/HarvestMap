...
--[[
-- TODO Use only pin type in this module. Avoid multiple convertation from id to type value.
-- TODO avoid usage of pin type value inside other modules.
 ]]--
...
local function RemovePinInAllControllers(nodeTag, pinTypeId)
    ...
    assert(nodeTag.counter, "Pin created without counter")
    HarvestDebugUtils.debug("Pin removed: " .. nodeTag.counter)
end
...
function Harvest.InitializeMapPinType(pinTypeId)
    ...
    if pinTypeId == Harvest.TOUR then
        ...
        local pve, pvp, imperial = LMP:AddPinFilter(pinType,
...
function Harvest.InitializeMapMarkers()
    ...
    LMP:AddPinFilter(Harvest.GetPinType("Debug"),
